...
`,Pt=[{name:"title",label:"Card Title",helper:"Shown at the top of the card.",selector:{text:{}}},{name:"default_expanded",label:"Expanded by Default",helper:"Whether the card opens expanded. Click the title to toggle.",selector:{boolean:{}}},{name:"aqi_entity",label:"AQI Sensor (Optional)",helper:"If set, displays this sensor's value with EPA AirNow bands. If empty or unavailable, falls back to a calculated score from PM2.5, VOC, and CO2.",selector:{entity:{domain:"sensor",device_class:"aqi"}}},{name:"temp_entity",label:"Temperature Sensor",helper:"Shown next to the headline. Also feeds the optional history graph.",selector:{entity:{domain:"sensor",device_class:"temperature"}}},{name:"humid_entity",label:"Humidity Sensor",helper:"Shown next to the headline. Also feeds the optional history graph.",selector:{entity:{domain:"sensor",device_class:"humidity"}}},{name:"pm1_entity",label:"PM1.0 Sensor",helper:"Display only - does not contribute to the calculated score.",selector:{entity:{domain:"sensor",device_class:"pm1"}}},{name:"pm25_entity",label:"PM2.5 Sensor",helper:"Drives the score when PM2.5 levels are the worst pollutant.",selector:{entity:{domain:"sensor",device_class:"pm25"}}},{name:"pm4_entity",label:"PM4.0 Sensor",helper:"Display only - does not contribute to the calculated score.",selector:{entity:{domain:"sensor"}}},{name:"pm10_entity",label:"PM10 Sensor",helper:"Drives the score when PM10 levels are the worst pollutant.",selector:{entity:{domain:"sensor",device_class:"pm10"}}},{name:"voc_entity",label:"VOC Index Sensor",helper:"Sensirion VOC Index (0-500 scale). Drives the score when VOCs are the worst pollutant.",selector:{entity:{domain:"sensor"}}},{name:"co2_entity",label:"CO2 Sensor (ppm)",helper:"Drives the score when CO2 is the worst pollutant (impacts cognitive function above 2000 ppm).",selector:{entity:{domain:"sensor",device_class:"carbon_dioxide"}}},{name:"nox_entity",label:"NOX Sensor",helper:"Nitrogen Oxides (NO2/NOX). Drives the score when NOX levels are high.",selector:{entity:{domain:"sensor"}}}];let Ot=class extends rt{setConfig(t){this._config=t}get _schema(){return this.hass?(this.hass.states!==this._cachedHassStates&&(this._cachedHassStates=this.hass.states,this._cachedVocEntities=Object.keys(this.hass.states).filter(t=>{if(!t.startsWith("sensor."))return!1;const e=this.hass.states[t].attributes.friendly_name?.toLowerCase()||"";return t.toLowerCase().includes("voc")||e.includes("voc")}),this._cachedNoxEntities=Object.keys(this.hass.states).filter(t=>{if(!t.startsWith("sensor."))return!1;const e=this.hass.states[t],i=e.attributes.friendly_name?.toLowerCase()||"",s=e.attributes.device_class?.toLowerCase()||"";return t.toLowerCase().includes("nox")||t.toLowerCase().includes("no2")||i.includes("nox")||i.includes("no2")||s.includes("nitrogen")}),this._cachedSchema=Pt.map(t=>"voc_entity"===t.name?{...t,selector:{entity:{...t.selector.entity,include_entities:this._cachedVocEntities}}}:"nox_entity"===t.name?{...t,selector:{entity:{...t.selector.entity,include_entities:this._cachedNoxEntities}}}:t)),this._cachedSchema||Pt):Pt}render(){return this.hass&&this._config?I`
    ...
    `:I``}_valueChanged(t){this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t.detail.value},bubbles:!0,composed:!0}))}};t([ht({attribute:!1})],Ot.prototype,"hass",void 0),t([pt()],Ot.prototype,"_config",void 0),Ot=t([lt("air-quality-card-editor")],Ot);console.info("%c  AIR-QUALITY-CARD  %c  Version 0.1.1  ","color: white; font-weight: bold; background: #03a9f4","color: #03a9f4; font-weight: bold; background: white");const Nt={aqi_entity:["sensor.","air_quality."],temp_entity:["sensor."],humid_entity:["sensor."],pm1_entity:["sensor."],pm25_entity:["sensor."],pm4_entity:["sensor."],pm10_entity:["sensor."],voc_entity:["sensor."],co2_entity:["sensor."]};let qt=class extends rt{constructor(){super(...arguments),this._expanded=!0,this._graphConfigured=!1,this._expandedUserDriven=!1}static{this.styles=Mt}static getConfigElement(){return document.createElement("air-quality-card-editor")}static getStubConfig(){return{title:"Living Room",default_expanded:!0,aqi_entity:"",temp_entity:"",humid_entity:"",pm1_entity:"",pm25_entity:"",pm4_entity:"",pm10_entity:"",voc_entity:"",co2_entity:""}}setConfig(t){if(!t||"object"!=typeof t)throw new Error("Invalid configuration: expected an object");for(const[e,i]of Object.entries(Nt)){const s=t[e];if(null!=s&&""!==s&&("string"!=typeof s||!i.some(t=>s.startsWith(t))))throw new Error(`${e} must be one of (${i.map(t=>t+"*").join(", ")}), got: ${String(s)}`)}this._config=t,this._expandedUserDriven||(this._expanded=!1!==t.default_expanded),this._setupGraphCard(t)}getCardSize(){return 5}t(t){return function(t,e="en"){const i=e=>{const i=t.split(".").reduce((t,e)=>t&&"object"==typeof t?t[e]:void 0,e);return"string"==typeof i?i:void 0};return i(Et[e])??i(Et.en)??t}(t,this.hass?.locale?.language||this.hass?.language||"en")}_safeNum(t){if(!t||!this.hass?.states[t])return null;const e=this.hass.states[t].state;if("unavailable"===e||"unknown"===e)return null;const i=parseFloat(e);return isNaN(i)?null:i}_formatNum(t,e=1){return null==t?"--":t.toFixed(e)}_getUnit(t,e){return t&&this.hass?.states[t]&&this.hass.states[t].attributes.unit_of_measurement||e}_setupGraphCard(t){this._graphCard||(this._graphCard=document.createElement("mini-graph-card"));const e=[];if(t.temp_entity&&e.push({entity:t.temp_entity,name:"Temp",color:"#fde68a"}),t.humid_entity&&e.push({entity:t.humid_entity,name:"Humidity",color:"#a8c0e0",y_axis:"secondary"}),0===e.length)return void(this._graphCard.style.display="none");const i=customElements.whenDefined("mini-graph-card"),s=new Promise((t,e)=>setTimeout(()=>e(new Error("mini-graph-card not installed")),2e3));Promise.race([i,s]).then(()=>{try{this._graphCard.setConfig({type:"custom:mini-graph-card",entities:e,hours_to_show:24,points_per_hour:2,line_width:2,animate:!0,smoothing:!0,hour24:!0,height:60,show:{name:!1,icon:!1,state:!1,legend:!0,labels:!1,fill:"fade"}})}catch(t){throw t}this._graphConfigured=!0,this._graphCard.style.display="block",this.hass&&(this._graphCard.hass=this.hass)}).catch(t=>{this._graphConfigured=!1,this._graphCard&&(this._graphCard.style.display="none");(t instanceof Error?t.message:String(t)).includes("not installed")?console.info("[air-quality-card] mini-graph-card not found, temp/humidity history graph disabled. Install via HACS to enable it."):console.warn("[air-quality-card] mini-graph-card setConfig failed:",t)})}updated(t){t.has("hass")&&this._graphCard&&this._graphConfigured&&this.hass&&(this._graphCard.hass=this.hass)}_toggle(){this._expanded=!this._expanded,this._expandedUserDriven=!0}_onKey(t){"Enter"!==t.key&&" "!==t.key||(t.preventDefault(),this._toggle())}render(){if(!this._config||!this.hass)return I``;const t=this._config,e=t.aqi_entity?this.hass.states[t.aqi_entity]:null,i=e?parseFloat(e.state):NaN,s=!!e&&!isNaN(i),o=s?e.attributes.friendly_name||this.t("topName.aqi"):this.t("topName.score"),a=this._safeNum(t.pm1_entity),r=this._safeNum(t.pm25_entity),n=this._safeNum(t.pm4_entity),l=this._safeNum(t.pm10_entity),d=this._safeNum(t.voc_entity),c=this._safeNum(t.co2_entity),h=this._safeNum(t.temp_entity),p=this._safeNum(t.humid_entity),u=this._safeNum(t.nox_entity),m=this._getUnit(t.temp_entity,"°C"),g=this._getUnit(t.humid_entity,"%"),v=this._getUnit(t.pm1_entity,"µg/m³"),f=this._getUnit(t.pm25_entity,"µg/m³"),_=this._getUnit(t.pm4_entity,"µg/m³"),y=this._getUnit(t.pm10_entity,"µg/m³"),x=this._getUnit(t.voc_entity,"index"),$=this._getUnit(t.co2_entity,"ppm"),b=this._getUnit(t.nox_entity,"index"),w=2*Math.PI*42;let A,C,S,E,U,M,P;if(s){A=Math.round(i),P=this.t("ring.aqi");const t=Ct.find(t=>i<=t.max);E=t.color,U=t.text,C=t.label,S=t.advice;M=w-Math.min(Math.max(i,0)/500,1)*w}else{const e=function({pm25:t,pm10:e,voc:i,voc_unit:s,voc_thresholds:o,voc_baseline:a,co2:r,nox:n,nox_unit:l,nox_thresholds:d,nox_baseline:c}){const h=o??$t(s),p=d??bt(l),u=a??(o?0:wt(s)),m=c??(d?0:At(l)),g=Math.max(1e-9,h.high-u),v=Math.max(1e-9,p.high-m),f=[{value:t,limit:75},{value:e,limit:150},{value:null!=i?Math.max(0,i-u):null,limit:g},{value:null!=r?r-400:null,limit:1600},{value:null!=n?Math.max(0,n-m):null,limit:v}].filter(t=>null!=t.value);if(0===f.length)return{score:null,label:"No data",color:"var(--air-quality-card-no-data-color, #9ca3af)",text:"var(--secondary-text-color)",advice:"Configure PM2.5, PM10, VOC, or CO₂ sensors to see a calculated score.",pct:0};const _=f.map(t=>Math.min(100,Math.max(0,t.value/t.limit*100))),y=Math.max(..._),x=Math.round(100-y),$=St.find(t=>x>=t.min);return{score:x,label:$.label,color:$.color,text:$.text,advice:$.advice,pct:x/100}}({pm25:r,pm10:l,voc:d,voc_unit:x,voc_thresholds:t.voc_thresholds,voc_baseline:t.voc_baseline,co2:c,nox:u,nox_unit:b,nox_thresholds:t.nox_thresholds,nox_baseline:t.nox_baseline});A=null==e.score?"--":e.score,P=this.t("ring.score"),E=e.color,U=e.text,C=e.label,S=e.advice,M=w-e.pct*w}const O=xt,N=Ut(a,O.pm1.good,O.pm1.mod,O.pm1.high),q=Ut(r,O.pm25.good,O.pm25.mod,O.pm25.high),H=Ut(n,O.pm4.good,O.pm4.mod,O.pm4.high),k=Ut(l,O.pm10.good,O.pm10.mod,O.pm10.high),T=t.voc_thresholds??$t(x),z=t.voc_baseline??(t.voc_thresholds?0:wt(x)),R=Ut(d,T.good,T.mod,T.high,z),j=Ut(c,O.co2.good,O.co2.mod,O.co2.high),D=t.nox_thresholds??bt(b),L=t.nox_baseline??(t.nox_thresholds?0:At(b)),V=Ut(u,D.good,D.mod,D.high,L);("Good"===C||"Moderate"===C)&&(null!=d&&d>T.mod&&(S=this.t("advice.vocHigh")),null!=c&&c>1e3&&(S=this.t("advice.co2High")),null!=c&&c>1500&&(S=this.t("advice.co2VeryHigh")),null!=u&&u>D.mod&&(S=this.t("advice.noxHigh")));const W=s?e.attributes.unit_of_measurement||"AQI":"/ 100",G=`${t.title||"Air quality"}: ${C}, ${A} ${W}`.trim(),Q=`color-mix(in srgb, ${E} 12%, transparent)`,F=`color-mix(in srgb, ${E} 35%, transparent)`;return I`
      <ha-card>
        <div
          ...
          aria-label=${G+(this._expanded?". Activate to collapse.":". Activate to expand.")}
          @click=${this._toggle}
          @keydown=${this._onKey}
        >
          ${this._expanded?this._renderExpandedHeader(t,o,A,s,E,U,S,h,p,m,g,P,C,42,w,M,G):this._renderCollapsed(t,A,s,e,E,U,C,Q,F,G)}
        </div>
        ...
        ${this._expanded?this._renderBottom({pm1:a,pm25:r,pm4:n,pm10:l,voc:d,co2:c,nox:u,pm1Unit:v,pm25Unit:f,pm4Unit:_,pm10Unit:y,vocUnit:x,co2Unit:$,noxUnit:b,pm1S:N,pm25S:q,pm4S:H,pm10S:k,vocS:R,co2S:j,noxS:V}):B}
      </ha-card>
      ...
      <div class="collapsed-row">
        ...
        <div class="collapsed-meta" role="group" aria-label=${d}>
          <div class="collapsed-value">
            <span class="num" style=${yt({color:a})}>${e}</span>
            <span class="unit">${c}</span>
          </div>
          <div
            class="chip"
            style=${yt({"--chip-bg":n,"--chip-border":l,color:a})}
          >
            <span class="dot" style=${yt({background:o})} aria-hidden="true"></span>
            ${r}
          </div>
        </div>
      </div>
    `}_renderExpandedHeader(t,e,i,s,o,a,r,n,l,d,c,h,p,u,m,g,v){const f=!s&&"--"!==i;return I`
      ...
      <div class="expanded-row">
        <div class="expanded-text">
          ...
          <div class="headline" role="group" aria-label=${v}>
            <span class="num" style=${yt({color:a})}>${i}</span>
        ...
        >
          ...
          <div class="ring-center" aria-hidden="true">
            ...
            <div class="ring-bottom-text" style=${yt({color:a})}>
              ${s||"--"===i?p:`${i}%`}
            </div>
          </div>
        </div>
    ...
    `}_renderBottom(t){const e=(t,e,i,s)=>I`
      ...
      >
        <div class="tile-head" aria-hidden="true">
          ...
          <span class="tile-status" style=${yt({color:s.text})}>${s.label}</span>
        </div>
        ...
        <div
          ...
          aria-label="${t} level"
        >
          <div class="fill" style=${yt({left:`${s.left??0}%`,width:`${s.pct}%`,background:s.color})}></div>
        </div>
      </div>
    `;return I`
      <div class="bottom">
    ...
    `}};t([ht({attribute:!1})],qt.prototype,"hass",void 0),t([pt()],qt.prototype,"_config",void 0),t([pt()],qt.prototype,"_expanded",void 0),qt=t([lt("air-quality-card")],qt),window.customCards=window.customCards||[],window.customCards.push({type:"air-quality-card",name:"Air Quality Card",preview:!0,description:"A custom card displaying air quality metrics and an overall score."});export{qt as AirQualityCard};
